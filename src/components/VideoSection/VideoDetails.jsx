...
const VideoDetails = () => {
  ...
  const fetchSelectedVideoDetails = async () => {
    ...
    try {
      ...
      console.errpr(error, "error fetching selected data");
    }
  };
  ...
  const fetchChannelData = async () => {
    if (selectedVideoDetails?.snippet?.channelId) {
      ...
      try {
        ...
        //console.error(error, "error fetching video data");
      } finally {
        setLoading(false);
      }
    }
  ...
  const fetchVideoComments = async () => {
      ...
      try {
        ...
        console.error(error, "error fetching comment data");
      } finally {
        setLoading(false);
      }
  };
  ...
  //   console.log('video details', selectedVideoDetails)
    ...
    >
      <div className="w-full flex flex-col p-4 lg:flex-row lg:space-x-4">
        <div className="flex flex-col lg:w-[70%] px-4 py-3 lg:py-6 overflow-auto">
          <div className="h-[300px] md:h-[450px] lg:h-[500px] xl:h-[600px] lg:ml-0 mr-[-16px] lg:mr-0">
            <iframe
              width="100%"
              height="100%"
          ...
          {selectedVideoDetails && (
            <div className="mt-4 flex flex-col gap-6">
              ...
              <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center">
                <div className="flex item-center mb-4 lg:mb-0">
                  <img
                  ...
                  <button className="bg-black text-white font-semoibold px-6 py-2 lg:py-3 mt-2 lg:mt-0 ml-1 lg:ml-6 rounded-full">
                    Subscribe
                  </button>
                </div>
                <div className="flex items-center justify-between space-x-4">
                  <button
                    className={`flex items-center space-x-1 rounded-full px-3 py-1 md:px-6 md:py-3 ${
          ...
          <div className="mt-8">
            <p className={`${isDarkMode?"text-gray-200":"text-blck"} font-semibold text-lg`}>
            {formatViewCount(selectedVideoDetails?.statistics?.commentCount)} {" "}
            comments</p>
          </div>
          {comments?.map((comment)=>(
            <VideoComments comment = {comment}
              key = {comment.id}
            />
          ))}
        </div>
