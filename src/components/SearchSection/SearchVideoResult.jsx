...
const SearchVideoResult = () => {
  ...
  const fetchSearchVideos = async() =>{
    ...
    try {
      ...
      const videoDetailsResonse = await fetchApiForYoutubeData("videos",{
        part:"snippet, contentDetails, statistics",
        id:videoIds
      })
      //console.log(data.items)
      setSearchResult(videoDetailsResonse.items)
    } catch (error) {
      console.error(error, "Error searching videos")
    }finally{
      setLoading(false)
    }
  }
    ...
    <div className='p-4'>
      {
        searchResult?.map((result)=>(
          <div className='flex flex-col md:flex-row mb-8'>
            ...
            {formatViewCount(result?.statistics?.viewCount)} view . {formatPublishTime(result?.snippet?.publishedAt)}
            </div>
            <p className='mt-2'>{result?.snippet?.description.slice(0, 180)}</p>
            </div>
          </div>
        ))
      }
